...
/*   Updated: 2021/01/25 21:49:47 by tseo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	s_info->sprite_height = (int)fabs((SCREEN_HEIGHT / s_info->transform_y) / vDiv);
	s_info->sprite_width = (int)fabs((SCREEN_HEIGHT / s_info->transform_y) / uDiv);
}
...
void	calc_draw_pos_of_sprite(t_sprite_info *s_info)
{
		...
		if (s_info->draw_start_y < 0) s_info->draw_start_y = 0;
		s_info->draw_end_y = s_info->sprite_height / 2 + SCREEN_HEIGHT / 2;
		if (s_info->draw_end_y >= SCREEN_HEIGHT) s_info->draw_end_y = SCREEN_HEIGHT - 1;
		s_info->draw_start_x = -s_info->sprite_width / 2 + s_info->sprite_screen_x;
		if(s_info->draw_start_x < 0) s_info->draw_start_x = 0;
		s_info->draw_end_x = s_info->sprite_width / 2 + s_info->sprite_screen_x;
		if(s_info->draw_end_x >= SCREEN_WIDTH) s_info->draw_end_x = SCREEN_WIDTH - 1;
}
