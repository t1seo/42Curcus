...
t_bmph	bmp_get_header(t_scene *scene)
{
	...
	bmph.info_h.color_used = 0x00ffffff;
...
void	bmp_save(t_scene *scene)
{
	...
	image = scene->img;
	i = 0;
	while ( i < (image->line_length / 4) * bmph.info_h.height)
	{
		if (i % (image->line_length / 4) < bmph.info_h.width)
			write(fd, &image[i], 4);
		i++;
	}
	write(fd, scene->img->addr, bmph.info_h.size_image);
