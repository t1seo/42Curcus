...
/*   Updated: 2021/05/07 21:29:12 by tseo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(&g_info.forks[philo->fork_1]);
	print_message(philo, TAKEN_FORK, get_time());
	pthread_mutex_lock(&g_info.forks[philo->fork_2]);
	print_message(philo, TAKEN_FORK, get_time());
	print_message(philo, EATING, get_time());
	vsleep(g_info.time_to_eat);
	pthread_mutex_unlock(&g_info.forks[philo->fork_1]);
	pthread_mutex_unlock(&g_info.forks[philo->fork_2]);
	philo->num_of_eat += 1;
